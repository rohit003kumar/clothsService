...
const handleDetectLocation = () => {
  ...
  const handleShowAddressInput = (type: 'pickup' | 'delivery') => {
    setAddressType(type)
  ...
  const handlePlaceOrder = (finalPaymentMethod = null) => {
    // ✅ NEW: Validate addresses before placing order
    if (!pickupAddress) {
      alert('Please select a pickup address first')
      return
    }
    ...
    const newOrder = {
      ...
      deliveryAddress: deliveryAddress || pickupAddress, // Use pickup as delivery if no delivery address
...
{/* 
            ...
            <section className="address-management-section" style={{
              marginTop: '24px',
              padding: '20px',
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
                ...
                <div style={{ padding: '16px', backgroundColor: 'white', borderRadius: '8px', border: '1px solid #e2e8f0' }}>
                  <h4 style={{ marginBottom: '12px', color: '#475569', fontSize: '14px', fontWeight: '600' }}>
                    🏠 Pickup Address
                  </h4>
                  {pickupAddress ? (
                    <AddressDisplay 
                  ...
                  <button 
                    onClick={() => handleShowAddressInput('pickup')}
                    style={{
                      padding: '8px 16px',
                  ...
                  >
                    {pickupAddress ? 'Change Address' : 'Set Pickup Address'}
                  </button>
                </div>
                ...
                <div style={{ padding: '16px', backgroundColor: 'white', borderRadius: '8px', border: '1px solid #e2e8f0' }}>
                  ...
                  <button 
                    onClick={() => handleShowAddressInput('delivery')}
                    style={{
                      padding: '8px 16px',
                      backgroundColor: deliveryAddress ? '#10b981' : '#6b7280',
                      color: 'white',
                      border: 'none',
                      borderRadius: '6px',
                      fontSize: '14px',
                  ...
                  >
                    {deliveryAddress ? 'Change Address' : 'Set Different Address'}
                  </button>
                </div>
              </div>
            </section>
        ...
        {/* ✅ NEW: Address Input Modal */}
        {showAddressInput && (
          <EnhancedAddressInput
            onAddressSelect={handleAddressSelect}
            onClose={handleCloseAddressInput}
            currentLocation={customerLocation}
            title={`Set ${addressType === 'pickup' ? 'Pickup' : 'Delivery'} Address`}
            showDeliveryOptions={addressType === 'delivery'}
          />
        )}
          ...
          <div className="orders-page">
            ...
            <div className="orders-list">
              {orders.length === 0 ? (
                ...
                {/* ✅ NEW: Display structured addresses if available */}
                {order.pickupAddress && (
                  <div style={{ marginTop: '8px' }}>
                    <strong>🏠 Pickup:</strong> {order.pickupAddress.fullAddress}
                  </div>
                )}
                {order.deliveryAddress && order.deliveryAddress !== order.pickupAddress && (
                  <div style={{ marginTop: '4px' }}>
                    <strong>🚚 Delivery:</strong> {order.deliveryAddress.fullAddress}
                  </div>
                )}
                        <p>
