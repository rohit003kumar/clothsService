...
useEffect(() => {
  const fetchServices = async () => {
    ...
    try {
      ...
      // For all users, initially show all services
      // Location-based filtering will be handled by CustomerLocationManager
      const res = await axios.get("/api/product/all");
      setServices(res.data || []);
      ...
      console.error("❌ Error fetching user or services:", err);
      setIsLoggedIn(false);
      // Fallback to empty services on error
      setServices([]);
    }
  };
...
const filteredServices = services.filter((service) => {
 ...
 <div>
  <LaundryHome />
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 md:py-8">
        {/* Location Manager for Customers */}
        {isLoggedIn && userRole === 'customer' && (
          <CustomerLocationManager
            onLocationChange={(location) => {
              // Update customer location state
              if (location.lat !== 0 && location.lng !== 0) {
                setCustomerLocation(location);
              }
            }}
            onServicesFiltered={(filteredServices) => {
              // Update services based on location filtering
